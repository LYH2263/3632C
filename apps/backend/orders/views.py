...
class OrderListView(APIView):
    def get(self, request):
        ...
        if merchant_id:
            ...
            permission_error = require_merchant_permission(
                request, target_merchant_id, forbidden_message='无权操作该商家订单'
            )
    ...
    def patch(self, request, order_id: int):
        ...
        if next_status == 'canceled':
            if user.role == 'buyer':
                ...
                permission_error = require_merchant_permission(
                    request, order.merchant_id, forbidden_message='无权操作该商家订单'
                )
                if permission_error is not None:
                    return permission_error
        else:
